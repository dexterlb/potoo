module Ui.Builder exposing (toTree, propertyMap, parentMap)
...
import Contracts
import Contracts exposing (Contract, Properties, fetch, makeCallee, Pid, PropertyID)
...
toTree pid contracts properties  = case Dict.get pid contracts of
  ...
  Nothing -> let meta = { noMetaData | key = "root" } in
...
toTree_ c key pid contracts properties widgets = let metaMaker = getMetaData key c pid in let metaData = metaMaker properties in
  case c of
    Contracts.StringValue s -> simpleTree widgets key metaMaker [] (StringWidget metaData <| Contracts.SimpleString s)
    Contracts.IntValue x    -> simpleTree widgets key metaMaker [] (NumberWidget metaData <| Contracts.SimpleInt x)
    Contracts.FloatValue x  -> simpleTree widgets key metaMaker [] (NumberWidget metaData <| Contracts.SimpleFloat x)
    Contracts.BoolValue x   -> simpleTree widgets key metaMaker [] (BoolWidget   metaData <| Contracts.SimpleBool x)
    ...
    Contracts.Delegate { destination, data } ->
        let (widget, children, widgets2) = case Dict.get destination contracts of
          (Just subcontract) ->
            let (child, widgets3) = toTree_ subcontract "connected" destination contracts properties widgets in
              ...
              , [ child ]
              , widgets3)
          Nothing -> (BrokenWidget metaData pid, [], widgets)
        in simpleTree widgets2 key metaMaker children widget
    Contracts.MapContract d -> let (children, newWidgets) = Dict.map (\ subkey subcontract -> (subcontract, subkey, pid)) d
      ...
      in simpleTree newWidgets key metaMaker children (ListWidget metaData)
...
propertyWidget prop metaData = case (Contracts.stripType prop.propertyType) of
  Contracts.TFloat -> case Contracts.getMinMax prop of
    -- todo: parse type here instead of using barbaric getMinMax
    Just (min, max) -> SliderWidget <| Ui.Widgets.Slider.init metaData { min = min, max = max }
    Nothing         -> NumberWidget metaData Contracts.Loading
  Contracts.TInt    -> NumberWidget metaData Contracts.Loading
  Contracts.TString -> StringWidget metaData Contracts.Loading
  Contracts.TBool   -> BoolWidget metaData Contracts.Loading
  _                 -> UnknownWidget metaData Contracts.Loading

propertyMap : Properties -> Widgets -> Dict (Pid, PropertyID) WidgetID
propertyMap properties (w, _) = w
  |> Dict.toList
