defmodule CacheTest do
  ...
  defmodule CacheTest.FooService do
    ...
    def handle_call({"methods.add_child", child = %Potoo.Contract.Delegate{}}, _, state) do
      %{children: children, contract_chan: chan} = state

      new_state = %{ state | children: [child | children] }

      Potoo.Channel.send(chan, new_state.children)
    ...
    defp contract(children) do
      %{
        ...
        "methods" => %{
          ...
          "add_child" => %Potoo.Contract.Function{
          ...
          "del_child" => %Potoo.Contract.Function{
  ...
  test "can get contract" do
    ...
    assert Cache.get_contract(cache, service) == Potoo.get_contract(service)
  end
  ...
  test "can subscribe to contract" do
    ...
    chan_b =  Potoo.subscribe_contract(service)

    :ok = Channel.subscribe(chan_a, self(), :cache)
    :ok = Channel.subscribe(chan_b, self(), :potoo)

    nil = Potoo.deep_call(service, ["methods", "add_child"], %Delegate{
      destination: child
    })
    ...
    potoo_contract = receive do
      {:potoo, data} -> data
    end

    assert cache_contract == potoo_contract
  end
  ...
  test "subscribing to subcontract reacts to changing nested contracts" do
    ...
    nil = Potoo.deep_call(service, ["children", 0, "methods", "add_child"], %Delegate{
    ...
    nil = Potoo.deep_call(service, ["methods", "del_child"], 0)
  ...
  test "can call function which is available on the root" do
    ...
    assert(
      ...
      ==
      Potoo.deep_call(service, ["methods", "hello"], %{"item" => "foo"})
    )
  end
  ...
  test "can call function on a child" do
    ...
    assert(
      ...
      ==
      Potoo.deep_call(service, ["children", 0, "methods", "hello"], %{"item" => "foo"})
    )
  end
end
