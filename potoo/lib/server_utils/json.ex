defmodule Potoo.ServerUtils.Json do
  alias Potoo.ServerUtils.PidCache
  require OK
  import OK, only: ["~>>": 2]

  def jsonify(%Potoo.Contract.Function{name: name, argument: argument, retval: retval, data: data}, pc) do
    %{
      "__type__" => "function",
      "name" => name,
  ...
  def jsonify(%Potoo.Contract.Delegate{destination: destination, data: data}, pc) do
    %{
      "__type__" => "delegate",
  ...
  def jsonify({Potoo.Channel, chan_pid}, pc) do
    %{
      "__type__" => "channel",
